...
STORES = ['セブンイレブン', 'ローソン', 'ファミリーマート', 'ナチュラルローソン', 'ローソンストア100', 'ミニストップ', 'デイリーヤマザキ', 'NewDays', 'セイコーマート', 'ポプラ']
STORE_GROUPS = {"コンビニ": STORES}
...
EXTRA_ROOTS = {'ナチュラルローソン': 'https://natural.lawson.co.jp/', 'ローソンストア100': 'https://store100.lawson.co.jp/', 'ミニストップ': 'https://www.ministop.co.jp/', 'デイリーヤマザキ': 'https://www.daily-yamazaki.jp/', 'NewDays': 'https://retail.jr-cross.co.jp/newdays/', 'セイコーマート': 'https://www.seicomart.co.jp/', 'ポプラ': 'https://www.poplar-cvs.co.jp/'}
...
class CatalogCollector:
    def __init__(self,path):
        ...
        with self.db() as db:
            ...
            record=db.execute('SELECT data FROM job WHERE id=1').fetchone()
            if record:
                job=json.loads(record[0])
                if any(store not in STORES for store in job.get('counts',{})):
                    job['counts']={k:v for k,v in job['counts'].items() if k in STORES}
                    job['pending']=[task for task in job.get('pending',[]) if task[0] in STORES]
                    job['current_store']=job['pending'][0][0] if job['pending'] else None
    ...
    def start(self,stores,limit,resume=False):
        with self.lock:
            with self.db() as db:
                ...
                db.execute('INSERT OR REPLACE INTO job VALUES (1,?)',(json.dumps(j,ensure_ascii=False),))
            try:
                self.thread=threading.Thread(target=self.run,args=(j,),daemon=True,name='public-product-collector')
                self.thread.start()
            except Exception:
                j['status']='中断（再開できます）'
                self.save(j)
...
def main():
    ...
    if page=='💾 保存・復元':
        ...
        if f and st.button('復元する'):
            try:
                ...
                data=json.load(f); rows=validate([d for d in data['catalog'] if isinstance(d,dict) and d.get('store') in STORES]); ids={d['id'] for d in rows}
    ...
    if page=='＋ 商品を追加・編集':
        ...
        with st.form('edit_'+(d.get('id') or 'new')):
            ...
            note=st.text_input('メモ・販売地域',value=d['note'])
